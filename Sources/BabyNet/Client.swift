...
public protocol BabyNetClientProtocol {
    func execute(callback: @escaping (Result<Data, Error>) -> ()) -> URLSessionTask?
}


public struct BabyNetClient: BabyNetClientProtocol {
    ...
    public func execute(callback: @escaping (Result<Data, Error>) -> ()) -> URLSessionTask? {
        
        var request: URLRequest
        var session: URLSession
        do {
            request = try self.requestConfig.createRequest()
            session = self.sessionConfig.createSession()
